...
namespace DotRecast.Recast
{
    ...
    public static class RcAreas
    {
            ...
            RcCompactHeightfield compactHeightfield)
        {
            ...
            Vector3 bmin = verts.ToVec3();
            Vector3 bmax = verts.ToVec3();
            for (int i = 3; i < verts.Length; i += 3)
            {
                bmin = Vector3.Min(bmin, verts.ToVec3(i));
                bmax = Vector3.Max(bmax, verts.ToVec3(i));
            }
        ...
        public static int OffsetPoly(float[] verts, int numVerts, float offset, float[] outVerts, int maxOutVerts)
        {
            ...
            for (int vertIndex = 0; vertIndex < numVerts; vertIndex++)
            {
                ...
                Vector3 vertA = verts.ToVec3(vertIndexA * 3);
                Vector3 vertB = verts.ToVec3(vertIndexB * 3);
                Vector3 vertC = verts.ToVec3(vertIndexC * 3);
