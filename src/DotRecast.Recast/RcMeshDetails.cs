...
namespace DotRecast.Recast
{
    ...
    public static class RcMeshDetails
    {
        ...
        public static float DistToTriMesh(Vector3 p, float[] verts, int nverts, List<int> tris, int ntris)
        {
            ...
            {
                Vector3 va = verts.ToVec3(tris[i * 4 + 0] * 3);
                Vector3 vb = verts.ToVec3(tris[i * 4 + 1] * 3);
                Vector3 vc = verts.ToVec3(tris[i * 4 + 2] * 3);
        ...
        public static int CompleteFacet(RcContext ctx, float[] pts, int npts, List<int> edges, int maxEdges, int nfaces, int e)
        {
            ...
            for (int u = 0; u < npts; ++u)
            {
                ...
                Vector3 vs = pts.ToVec3(s * 3);
                Vector3 vt = pts.ToVec3(t * 3);
                Vector3 vu = pts.ToVec3(u * 3);
            ...
            ref List<int> tris, ref List<int> edges, ref List<int> samples)
        {
            ...
            if (sampleDist > 0)
            {
                ...
                Vector3 bmin = @in.ToVec3();
                Vector3 bmax = @in.ToVec3();
                for (int i = 1; i < nin; ++i)
                {
                    bmin = Vector3.Min(bmin, @in.ToVec3(i * 3));
                    bmax = Vector3.Max(bmax, @in.ToVec3(i * 3));
                }
